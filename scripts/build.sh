...
echo "--------- BLACK ---------------------------"
echo " "
if ! black app tests --config pyproject.toml --verbose 2>&1 | tee "$BLACK_PATH/black_output.txt"; then
...
echo "--------- PYLINT ---------------------------"
echo " "
if ! pylint --rcfile pyproject.toml app tests --verbose 2>&1 | tee "$PYLINT_PATH/pylint_output.txt"; then
